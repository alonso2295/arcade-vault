// ===== lib/games/asteroids/engine.ts — Asteroids game engine (ported from references/templates/started-games/02-asteroids/game.js) =====
...
export function createAsteroidsGame(
  ...
  let lives = 3;
  let level = 1;
  let state: AsteroidsState["state"] = "playing";
  let deadTimer = 0;
  ...
  let lastLives = -1;
  let lastLevel = -1;
  let lastState: AsteroidsState["state"] | null = null;

  function notifyStateChange() {
    if (score !== lastScore || lives !== lastLives || level !== lastLevel || state !== lastState) {
      lastScore = score;
      lastLives = lives;
      lastLevel = level;
      lastState = state;
      onStateChange({ score, lives, level, state });
    }
  }
