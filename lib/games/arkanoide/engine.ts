// ===== lib/games/arkanoide/engine.ts — Arkanoide game engine (ported from references/templates/started-games/04-arkanoid/) =====
...
// ===== spritesheet helpers (ported from assets/spritesheet.js) =====
...
export function createArkanoideGame(
  canvas: HTMLCanvasElement,
  onStateChange: (s: ArkanoideState) => void
): { destroy(): void } {
  const ctx = canvas.getContext("2d")!;

  // ----- spritesheet loading (scoped to this instance) -----
  ...
  function loadSpritesheet(cb: () => void) {
    ...
    rawImg.src = "/games/arkanoide/spritesheet-breakout.png";
  }
  ...
  function drawSprite(name: "paddle" | "ball" | `block_${BlockColor}`, x: number, y: number, w: number, h: number) {
    if (!ssLoaded) return;
  ...
  // ----- audio -----
  const bounceSound = new Audio("/games/arkanoide/sounds/ball-bounce.mp3");
  const breakSound = new Audio("/games/arkanoide/sounds/break-sound.mp3");
  ...
  let lives = 3;
  let score = 0;
  let gameState: ArkanoideState["state"] = "playing";
  let currentLevel = 1;
  let isPaused = false;
  ...
  // ----- pause overlay level-select buttons -----
  ...
  let lastLives = -1;
  let lastLevel = -1;
  let lastState: ArkanoideState["state"] | null = null;
  let lastPaused: boolean | null = null;

  function notifyStateChange() {
    const reportedState: ArkanoideState["state"] = isPaused && gameState === "playing" ? "paused" : gameState;
    if (
      score !== lastScore ||
      lives !== lastLives ||
      currentLevel !== lastLevel ||
      reportedState !== lastState ||
      isPaused !== lastPaused
    ) {
      lastScore = score;
      lastLives = lives;
      ...
      lastPaused = isPaused;
      onStateChange({ score, lives, level: currentLevel, state: reportedState });
    }
  }
