...
import FeatureIcon, { type FeatureIconKind } from "@/components/FeatureIcon";
...
const TICK_TIMES = ["hace 2 min", "hace 5 min", "hace 8 min", "hace 12 min", "hace 18 min", "hace 24 min", "hace 31 min"];
const TICK_COLORS = ["magenta", "yellow", "green", "cyan", "cyan", "green", "yellow"];

export default function GamesLandingPage() {
  useReveal();
  const router = useRouter();

  const tickRows = seededScores(1, 7).map((row, i) => ({
    ...row,
  ...
  const topRows = seededScores(2, 5);
